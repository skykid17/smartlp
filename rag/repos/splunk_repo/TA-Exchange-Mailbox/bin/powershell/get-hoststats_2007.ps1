...
function Output-HostData($Server) {
	...
	$ExServer = (Get-ExchangeServer -Identity $Server.Name)
	$Product  = (Get-Command ExSetup|%{$_.FileVersionInfo})
	$IsActive = "Disabled"
	if ($ExServer.IsMemberOfCluster -eq 'Yes') {
		if ((Get-ClusteredMailboxServerStatus).OperationalMachines | select-string $env:ComputerName | select-string "active") {
			$IsActive = "Active"
		} else {
			$IsActive = "Passive"
		}
	...
	$TestServiceHealth = (Test-ServiceHealth)
	$ProcsOK = $true
	...
	[void]$Output.Add("Name=`"$env:ComputerName`"")
