...
class RelayModAction(JsonModularInput):
    ...
    def __init__(self, logger):
        ...
        args = [
            ...
            Field("client_cert", "Client Certificate", "Filename of client certificate for verification.",
                  required_on_create=False, required_on_edit=False),
            Field("splunk_cloud_domains", "Splunk Cloud Domains", "List of Splunk Cloud Domains.",
                  required_on_create=False, required_on_edit=False)
        ]
    ...
    def run(self, params):
        ...
        cloud_domains = json.loads(params['splunk_cloud_domains'])
        self.logger.info("splunk cloud domains: %s", cloud_domains)
            ...
            with self.ensure_local_dispatch_dir(stanza_name, sid) as dispatch_dir:
                try:
                    if is_cloud:
                        ...
                        if server_host in target:
                            # Valid server_host already specified in Cloud stack URI
                            target_new = target

                        elif self.SEARCH_HEAD_ID_PREFIX in target:
                            # Invalid server_host specified within URI (target)
                            ...
                            target_new = ''.join([url_left, server_host, url_right])
                            self.logger.warn("Invalid AR server_host specified in URI: %s", sh_match)
                        else:
                            # Generic Cloud Stack URI provided
                            # Augment/update URL with AR action specified SH-ID (server_host)
                            url_separator_pattern = '://'
                            url_left = target[:target.find(url_separator_pattern) + len(url_separator_pattern)]
                            url_right = target[target.find(url_separator_pattern) + len(url_separator_pattern):]
                            target_new = ''.join([url_left, server_host, '.', url_right])
                    else:
                        # URI: non-Splunk Cloud deployment
                        target_new = target

                    self.logger.info("Remote URI - is cloud: %s, server_host in URI: %s", is_cloud, (server_host in target))
                    self.logger.info("AR Relay - params: %s", str(params))
                    self.logger.info("AR Relay - old target: %s", target)
                    self.logger.info("AR Relay - new target: %s", target_new)
                    results_file = self.fetch_results(target_new, key, dispatch_dir, file_name, worker, password, verify, client_cert)
                except HTTPError as e:
                    self.logger.error("Failed to fetch results: %s", e)
                    if e.response.status_code == requests.codes.not_found:
                        self.dequeue(target_new, key, worker, password, verify, client_cert)
                        self.logger.info("Dequeued missing result with key: %s, file_name: %s", key, file_name)
                except Exception as e:
                    self.logger.error("Unknown Error - failed to fetch results: %s", e)
                    raise
                ...
                if results_file is not None:
                    ...
                    payload['relay_session_key'] = session_key
