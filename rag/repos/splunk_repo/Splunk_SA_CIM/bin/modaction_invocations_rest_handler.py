...
class ModularActionInvocationsRestHandler(
    ...
    def run_search(self, sid, rid, earliest_time, latest_time, session_key=None):
        """
        ...
        srch = splunk.search.dispatch(search_string, earliestTime=earliest_time, latestTime=latest_time, sessionKey=session_key)
    ...
    def get_modaction_invocations(self, sid, rid, earliest_time, latest_time, session_key=None):
        """
        ...
        try:
            return self.response(
                self.run_search(
                    valid_sid, valid_rid, earliest_time, latest_time, session_key),
                http_client.OK)
    ...
    def handle_get(self, args):
        ...
        sid = query.pop('sid')
        rid = query.pop('rid')
        earliest_time = query.pop('earliest', None)
        latest_time = query.pop('latest', None)

        return self.get_modaction_invocations(sid, rid, earliest_time, latest_time, session_key)
