...
class HECConfig(object):
    ...
    def __init__(self, splunkd_uri, session_key):
        self._conf_mgr = cm.ConfManager(
            splunkd_uri, session_key, app_name="splunk_httpinput"
        )
    ...
    def update_settings(self, settings):
        ...
        try:
            self._conf_mgr.update_data_input(self.input_type, self.input_type, settings)
        except Exception:
            logger.error(
                "Failed to update httpinput settings, reason=%s", traceback.format_exc()
            )
            raise
    ...
    def create_http_input(self, stanza):
        ...
        try:
            self._conf_mgr.create_data_input(self.input_type, stanza["name"], stanza)
        except req.ConfExistsException:
            pass
        except Exception:
            logger.error(
                "Failed to create httpinput=%s, reason=%s",
                stanza["name"],
    ...
    def update_http_input(self, stanza):
        ...
        self._conf_mgr.update_data_input(self.input_type, stanza["name"], stanza)
    ...
    def delete_http_input(self, name):
        ...
        try:
            ...
            logger.error(
                "Failed to delete httpinput=%s, reason=%s", name, traceback.format_exc()
            )
            raise
    ...
    def get_http_input(self, name):
        ...
        try:
            ...
            logger.error(
                "Failed to get httpinput=%s, reason=%s", name, traceback.format_exc()
            )
            raise
...
if __name__ == "__main__":
    ...
    for host in hosts:
        session_key = cred.CredentialManager.get_session_key("admin", "admin", host)
        ...
        new_limit = {"max_content_length": int(limits["max_content_length"]) * 2}
        hic.set_limits(new_limit)
