...
"""
This is the main entry point for Resource Metadata (Cloud Storage) Input.
"""
...
def print_scheme():
    ...
    description = (
        "Collect and index Google Cloud Resource Metadata data for cloud storage"
    )
...
def run():
    """
    Main loop. Run this Input forever
    """
    logger.info("Start google_resource_metadata_cloud_storage")
    metas, tasks = tacommon.get_configs(
        mconf.GoogleResourceMetadataCloudStorageConfig,
        "google_resource_metadata_cloud_storage",
        logger,
    )

    if not tasks:
        logger.info("No Tasks to process for resource metadata cloud storage input")
        return
    ...
    conf_monitor = tacommon.create_conf_monitor(
        conf_change_handler, [grc.storage_data_collection_conf]
    )
    loader_mgr.add_timer(conf_monitor, time.time(), interval=10)
    jobs = [grdl.StorageResourceDataLoader(task) for task in tasks]
    loader_mgr.start(jobs)

    logger.info("End google_resource_metadata_cloud_storage")
...
def main():
    ...
    logging.setup_root_logger(
        "splunk_ta_google-cloudplatform", "google_cloud_resource_metadata_cloud_storage"
    )
