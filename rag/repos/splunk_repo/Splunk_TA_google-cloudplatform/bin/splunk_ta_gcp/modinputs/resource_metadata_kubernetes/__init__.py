...
"""
This is the main entry point for Resource Metadata (Kubernetes) Input.
"""
...
def print_scheme():
    ...
    description = "Collect and index Google Cloud Resource Metadata data for kubernetes"
...
def run():
    ...
    """
    logger.info("Start google_resource_metadata_kubernetes")
    metas, tasks = tacommon.get_configs(
        mconf.GoogleResourceMetadataKubernetesConfig,
        "google_resource_metadata_kubernetes",
        logger,
    )

    if not tasks:
        logger.info("No Tasks to process for resource metadata kubernetes input")
        return
    ...
    conf_monitor = tacommon.create_conf_monitor(
        conf_change_handler, [grc.kubernetes_data_collection_conf]
    )
    loader_mgr.add_timer(conf_monitor, time.time(), interval=10)
    jobs = [grdl.KubernetesResourceDataLoader(task) for task in tasks]
    loader_mgr.start(jobs)

    logger.info("End google_resource_metadata_kubernetes")
...
def main():
    ...
    logging.setup_root_logger(
        "splunk_ta_google-cloudplatform", "google_cloud_resource_metadata_kubernetes"
    )
