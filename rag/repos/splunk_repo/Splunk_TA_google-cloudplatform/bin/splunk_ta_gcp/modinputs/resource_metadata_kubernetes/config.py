...
class GoogleResourceMetadataKubernetesConfig(gconf.GoogleConfig):
    """
    ...
    conf_file = "google_cloud_resource_metadata_inputs_kubernetes"
    conf_file_w_path = op.join(app_dir, "local", conf_file + ".conf")
    passwords = "passwords"
    passwords_file_w_path = op.join(app_dir, "local", passwords + ".conf")

    def __init__(self):
        super(GoogleResourceMetadataKubernetesConfig, self).__init__(
            grc.google_resource_metadata_kubernetes
        )

    @staticmethod
    def data_collection_conf():
        return grc.kubernetes_resource_settings_conf
    ...
    def _expand_tasks(self, stanza, creds):
        ...
        for api_interval in stanza[grc.apis].split(","):
            ...
            _task = {
                ...
                ggc.google_location: stanza.get(ggc.google_location),
            }
