...
"""
This is the main entry point for Resource Metadata Input.
"""
...
def run():
    ...
    if not tasks:
        logger.info("No Tasks to process for resource metadata compute engine input")
        return

    logger.debug("Received " + str(len(tasks)) + " Tasks.")
    ...
    conf_monitor = tacommon.create_conf_monitor(
        conf_change_handler, [grc.compute_data_collection_conf]
    )

    time.sleep(5)
    loader_mgr.add_timer(conf_monitor, time.time(), 10)
    jobs = [grdl.ComputeResourceDataLoader(task) for task in tasks]
