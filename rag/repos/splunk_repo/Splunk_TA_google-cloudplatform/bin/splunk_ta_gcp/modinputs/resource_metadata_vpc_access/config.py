...
class GoogleResourceMetadataVpcAccessConfig(gconf.GoogleConfig):
    """
    ...
    conf_file = "google_cloud_resource_metadata_inputs_vpc_access"
    conf_file_w_path = op.join(app_dir, "local", conf_file + ".conf")
    passwords = "passwords"
    passwords_file_w_path = op.join(app_dir, "local", passwords + ".conf")

    def __init__(self):
        super(GoogleResourceMetadataVpcAccessConfig, self).__init__(
            grc.google_resource_metadata_vpc_access
        )

    @staticmethod
    def data_collection_conf():
        return grc.vpcaccess_resource_settings_conf
