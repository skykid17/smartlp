...
class ThreadPool:
    ...
    def __init__(self, min_size=1, max_size=128, task_queue_size=256, daemon=True):
    ...
    def _do_resize_according_to_loads(self):
        ...
        elif int(work_size / thr_size) < self._low_watermark:
            if thr_size > self._min_size:
                thr_size = max(self._min_size, int(thr_size / 2))
                self.resize(thr_size)
        self._last_resize_time = time()
    ...
    def _do_admin(self):
        ...
        while 1:
            try:
                wakup = admin_q.get(resize_win + 1)
            except queue.Empty:
    ...
    def _run(self):
        ...
        _LOGGER.debug("Worker thread %s stopped.", threading.current_thread().getName())
