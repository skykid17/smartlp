...
class KVClient(object):
    ...
    def _do_get_endpoint(self, app, owner, collection, key_id, uri_template):
        ...
        app = urlparse.quote(app.encode("utf-8"), "")
        owner = urlparse.quote(owner.encode("utf-8"), "")

        if collection:
            collection = urlparse.quote(collection.encode("utf-8"), "")

        if key_id:
            key_id = urlparse.quote(key_id.encode("utf-8"), "")
