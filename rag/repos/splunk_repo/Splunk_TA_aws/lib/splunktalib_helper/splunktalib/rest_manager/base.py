...
class BaseRestHandler(admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        user, app = self.user_app()
        # Eliminate unnecessary _reload calls
        ...
        if self.callerArgs.id is None:
            ...
            try:
                ...
                makeConfItem(
                    self.callerArgs.id, ent, confInfo, user=user, app=app
                )
    ...
    def decode(self, name, ent):
        ...
        #Adverse Key Mapping
        ent = {k: v for k, v in six.iteritems(ent)}
            ...
            for v in vs:
                if not self.validators[k].validate(v, args):
                    RestHandlerError.ctl(
                        1100,
                        msgx=('{msg} - field={k}'
                              .format(msg=self.validators[k].msg, k=k)),
                        logLevel=logging.INFO,
                    )
        return args
    ...
    def normalize(self, data):
        ...
        for k, vs in data.items():
            ...
            if isinstance(vs, list) or isinstance(vs, dict) or isinstance(vs, tuple):
                data[k] = [isinstance(v, six.string_types) and self.normalisers[k].normalize(v) or v for v in vs]
            else:
