...
from splunk import rest, ResourceNotFound
...
class CredMgmt(object):
    ...
    def _makeRequestURL(self, name, isNew=True):
        credId = '' if isNew else urlparse.quote('{realm}:{username}:'.format(realm=CredMgmt._escapeString(self._realm), username=CredMgmt._escapeString(name)))
