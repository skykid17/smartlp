...
#
"""
Configuration file for Splunk Instance.
"""
import glob
import json
...
class SplunkInfo:
    ...
    @staticmethod
    def validate(splunk):
        """Validates the splunk-info file."""
        if not isinstance(splunk, dict):
            logger.error(
                "Invalid splunk-info file. Expect splunk as a dict object", got=splunk
            )
        ...
        scheme, host, port = sutils.extract_http_scheme_host_port(splunk["mgmt_uri"])
        ...
        try:
            ...
            logger.error(
                "Invalid splunk-info file, credentials or URI are " "probably wrong",
                error=str(exc),
            )
            raise ValueError(  # pylint: disable=raise-missing-from
                "Invalid splunk-info file"
            )


class RestStanzaField:
    """Class for Rest Stanza field."""
    ...
    @staticmethod
    def validate(field):
        """Validates the REST api spec."""
        if not isinstance(field, dict):
            logger.error(
                "Invalid rest API spec, expect stanza_field as dict", got=field
            )
...
class RestAPISpec:
    """Class for Rest API spec."""

    def __init__(self, spec):
        self.validate(spec)
        self.name = spec["name"]
        self.endpoint = spec["endpoint"]
        self.stanza_fields = [RestStanzaField(field) for field in spec["stanza_fields"]]
    ...
    def validate(spec):
        ...
        if not isinstance(spec["stanza_fields"], list):
            logger.error(
                "Invalid rest API spec, expect list stanza_fields",
...
class HostStanza:
    ...
    def validate(host_stanza, **kwargs):
        ...
        if not isinstance(host_stanza, dict):
            logger.error(
                "Invalid config file. Expect stanza as a dict object", got=host_stanza
            )
        ...
        if not isinstance(rest_stanzas, list):
            logger.error(
                "Invalid config file. Expect list of stanzas", got=rest_stanzas
            )
            raise ValueError("Invalid config file")
...
def load_rest_api_specs(spec_dir):
    ...
    if not op.exists(spec_dir):
        logger.error(
            "Didn't find `rest_specs` directory. Need config that " "first",
            rest_spec_dir=spec_dir,
        )
...
def _do_get_spec(spec_file):
    with open(spec_file) as spec_fp:  # pylint: disable=unspecified-encoding
        try:
...
def load_configs(config_file, splunks, rest_api_specs, **kwargs):
    ...
    with logging.LogContext(config_file=config_file):
        return _do_load_configs(config_file, splunks, rest_api_specs, **kwargs)


def _do_load_configs(config_file, splunks, rest_api_specs, **kwargs):
    with open(config_file) as config_fp:  # pylint: disable=unspecified-encoding
        try:
            host_stanzas = json.load(config_fp)
        except ValueError as err:
            logger.error("Invalid config file. Invalid json")
            raise err

    if not isinstance(host_stanzas, list):
        logger.error("Invalid config file. Expect list of stanzas", got=host_stanzas)
        raise ValueError("Invalid config file")

    host_stanzas = [HostStanza(host_stanza, **kwargs) for host_stanza in host_stanzas]
...
def _validate_host_stanza_against_rest_specs(  # pylint: disable=invalid-name
    host_stanza, rest_api_specs
):
    """
    ...
    for stanza in host_stanza.stanzas:
        ...
        for key, required in valid_keys.items():
            if not required:
                continue
...
def _do_load_splunks(splunk_info_file):
    ...
    with open(splunk_info_file) as spl_fp:  # pylint: disable=unspecified-encoding
        try:
            splunks = json.load(spl_fp)
        except ValueError as err:
            logger.error("Invalid splunk-info file. Invalid json")
            raise err
