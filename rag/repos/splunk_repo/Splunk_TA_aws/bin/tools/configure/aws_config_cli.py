...
import splunksdc.log as logging  # isort:skip
...
def _setup_logger(logger):  # pylint: disable=redefined-outer-name
...
class AwsConfigCli:
    ...
    def __init__(self, params, appname, splunk_info_file, spec_dir):
        ...
        self.config_mgr = app_config.AppConfigManager(self.splunks, self.appname)
    ...
    def create(self):
        ...
        if self.params.dry_run:
            logger.info(
                "In dry run mode, only validate configurations. "
        ...
        for host_stanza in host_stanzas:
            ...
            with logging.LogContext(
                hostname=host_stanza.hostname, endpoint=endpoint, action="create"
            ):
                try:
                    self.config_mgr.create(
                        endpoint, host_stanza.hostname, host_stanza.stanzas
                    )
    ...
    def list(self):
        ...
        if self.params.names:
            names = self.params.names.split(",")
        results = self.config_mgr.list(self.endpoint, self.params.hostname, names)
        items = []
    ...
    def _get_endpoint(self):
        ...
        if not spec:
            logger.error(
                "No rest API spec is found for resource", resource=self.params.resource
            )
...
def _get_aws_addon_resources():
    return {
        ...
        "private-account": "AWS private account setting",
        "billing-cost": "modinput data input",
        "cloudtrail": "modinput data input",
        "cloudtrail-lake": "modinput data input",
        "cloudwatch": "modinput data input",
        "cloudwatch-logs": "modinput data input",
        "config": "modinput data input",
        "config-rules": "modinput data input",
        "incr-s3": "modinput data input",
        "inspector": "modinput data input",
        "inspector_v2": "modinput data input",
        "kinesis": "modinput data input",
        "metadata": "modinput data input",
        "s3": "modinput data input",
        "sqs-based-s3": "modinput data input",
        "sqs": "modinput data input",
    }


def _add_create_all_option(subparsers):
    help_msg = (
        "Create all resources specified in conf file which can contain"
    ...
    help_msg = (
        "Stanza configuration file, refer to config_examples/all.json " + "for example"
    )
...
def compose_cli_args():
    ...
    for resource, desc in resources.items():
        help_msg = "{} {} subcommand".format(  # pylint: disable=consider-using-f-string
            resource, desc
        )
        ...
        create_parser = action_subparsers.add_parser(
            "create",
            help="Create {}".format(desc),  # pylint: disable=consider-using-f-string
        )
        help_msg = (
        ...
        create_parser.add_argument(
            "--hostname", dest="hostname", required=True, help="Splunk hostname"
        )
        create_parser.add_argument(
            "--config-file", dest="config_file", required=True, help=help_msg
        )
        ...
        list_parser = action_subparsers.add_parser(
            "list",
            help="List {}".format(desc),  # pylint: disable=consider-using-f-string
        )
        ...
        help_msg = (
            "List {} by names. `names` are separated by `,`."  # pylint: disable=consider-using-f-string
            "For example, 's3_prod1,s3_prod2'. if omit this "
            "argument, list all"
        ).format(desc)
        list_parser.add_argument("--names", dest="names", required=False, help=help_msg)

        # delete
        delete_parser = action_subparsers.add_parser(
            "delete",
            help="Delete {}".format(desc),  # pylint: disable=consider-using-f-string
        )
        ...
        help_msg = (
            "Delete {} by names. `names` are separated by `,`."  # pylint: disable=consider-using-f-string
            "For example, 's3_prod1,s3_prod2'."
        ).format(desc)
        delete_parser.add_argument(
...
def main():
    """Main method for config cli file."""
