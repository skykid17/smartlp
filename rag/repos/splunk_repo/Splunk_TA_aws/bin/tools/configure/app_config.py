...
def get_session_key(splunk_info):
    ...
    scheme, host, port = sutils.extract_http_scheme_host_port(splunk_info.mgmt_uri)
    service = sc.connect(
        scheme=scheme,
        host=host,
        port=port,
...
class AppConfigException(Exception):
    """Class for App config exception."""

    def __init__(self, message, failures):
        super(  # pylint: disable=super-with-arguments
            AppConfigException, self
        ).__init__(message)
...
class AppConfigClient:
    ...
    def list(self, endpoint, stanza_name=None):
        ...
        if stanza_name:
            endpoint = "{}/{}".format(  # pylint: disable=consider-using-f-string
                endpoint, urllib.parse.quote(stanza_name, safe="")
            )

        response = self.rest_client.get(
            endpoint, owner=self.owner, app=self.appname, output_mode="json", count=-1
        )
        if response.status not in (200, 201):
            logger.error(
                "Failed to query",
                endpoint=endpoint,
                uri=self.splunk_info.mgmt_uri,
                reason=response.reason,
            )
            raise Exception(
                "Failed to query endpoint {}".format(  # pylint: disable=consider-using-f-string
                    response
                )
            )
...
class AppConfigManager:
    ...
    def create(self, endpoint, hostname, stanzas):
        ...
        for stanza in stanzas:
            try:
                ...
                logger.error("Failure", stanza=stanza["name"], error=str(exc))
            else:
    ...
    def delete(self, endpoint, hostname, stanza_names):
        ...
        for stanza_name in stanza_names:
            try:
                ...
                logger.error("Failure", stanza=stanza_name, error=str(exc))
            else:
    ...
    def _get_client(self, hostname):
        ...
        client = AppConfigClient(self.splunks[hostname], self.appname, self.owner)
