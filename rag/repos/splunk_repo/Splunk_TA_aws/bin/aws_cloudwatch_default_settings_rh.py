...
_CLOUDWATCH_DEFAULT_METRICS = {
    ...
    "AWS/Lambda": ["Duration", "Errors", "Invocations", "Throttles", "IteratorAge"],
    "AWS/Logs": [
        "DeliveryErrors",
        "DeliveryThrottling",
        "ForwardedBytes",
...
_CLOUDWATCH_DEFAULT_DIMENSIONS = {
    ...
    "AWS/ApplicationELB": [
        ...
        {"AvailabilityZone": [".*"], "TargetGroup": [".*"], "LoadBalancer": [".*"]},
    ],
    "AWS/Billing": [
        {"Currency": [".*"]},
        {"Currency": [".*"], "ServiceName": [".*"]},
        {"Currency": [".*"], "LinkedAccount": [".*"], "ServiceName": [".*"]},
    ],
    "AWS/CloudFront": [{"DistributionId": [".*"], "Region": "Global"}],
    "AWS/CloudSearch": [{"ClientId": [".*"], "DomainName": [".*"]}],
    "AWS/DynamoDB": [{"TableName": [".*"]}, {"TableName": [".*"], "Operation": [".*"]}],
    "AWS/EBS": [{"VolumeId": [".*"]}],
    "AWS/EC2": [
    ...
    "AWS/ElasticMapReduce": [{"JobFlowId": [".*"], "JobId": [".*"]}],
    "AWS/Events": [{"RuleName": [".*"]}],
    ...
    "AWS/Redshift": [{"ClusterIdentifier": [".*"]}, {"NodeID": [".*"]}],
    ...
    "AWS/SWF": [
        ...
        {"ActivityTypeName": [".*"], "ActivityTypeVersion": [".*"], "Domain": [".*"]},
        {"WorkflowTypeName": [".*"], "WorkflowTypeVersion": [".*"], "Domain": [".*"]},
    ],
    "AWS/StorageGateway": [
...
_CLOUDWATCH_DEFAULT_NAMESPACES = [
    "AWS/ApiGateway",
    "AWS/ApplicationELB",
    "AWS/Billing",
    "AWS/EBS",
...
def main():
    """Main method for Cloudwatch settings RH."""
