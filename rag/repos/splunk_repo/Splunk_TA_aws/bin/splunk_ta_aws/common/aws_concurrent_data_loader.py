...
#
"""
File for data handling in AWS TA.
"""
...
from multiprocessing import (  # isort: skip
    Manager,
    Process,
    cpu_count,
)

from six.moves import (  # isort: skip
    queue,
    range,
    zip,
)
...
def do_load_data(tear_down_q, task_configs):
    ...
    loader_mgr.add_timer(_wait_for_tear_down(tear_down_q, loader_mgr), time.time(), 3)
...
class AwsConcurrentDataLoader:
    """Class for AWS concurrent data loader manager."""
    ...
    def start(self):
        """Start method for AwsConcurrentDataLoader"""
        if self._started:
            return
        self._started = True
    ...
    def stop(self):
        """Exit method for AwsDataLoaderManager"""
...
class AwsDataLoaderManager:  # pylint: disable=too-many-instance-attributes
    """Class for AWS data loader manager."""
        ...
        for loader in loaders:
            try:
                ...
                logger.warn(  # pylint: disable=deprecated-method
                    "Wait timeout, terminate loader=%s, error=%s",
                    loader,
    ...
    def _devide_tasks(self, event_writer):
        ...
        tasks = [[] for _ in range(process_num)]
