...
class CredentialManager:
    ...
    def get_session_key(username, password, retry=3):
        ...
        raise Exception(
            "Failed to get session key %d: %s"  # pylint: disable=consider-using-f-string
            % (
                response.status,
                response.reason,
            )
    ...
    def update(
        self, realm, user, password, app, owner="nobody", retry=3
    ...
    def create(
        ...
        @return: The encrypted password value.
        """
        cred = SplunkStoredCredential(
            app, owner, user, host_path=self._host_path, sessionKey=self._session_key
        )
        cred.realm = realm
    ...
    def delete(
        self, realm, user, app, owner="nobody", retry=3
    ):  # pylint: disable=too-many-arguments
        """
        ...
        path = SplunkStoredCredential.build_id(
            realm_user, app, owner, host_path=self._host_path
        )
        for _ in range(retry):
            try:
                response, _ = sr.simpleRequest(
                    path, method="DELETE", sessionKey=self._session_key
                )
                ...
                _LOGGER.warning(
                    "Entity not found for (%s %s %s %s)", realm, user, app, owner
                )
            return True
        return False
    ...
    def get_clear_password(
        ...
        return self._get_credentials(realm, user, app, owner, "clear_password", retry)
    ...
    def get_encrypted_password(
        ...
        return self._get_credentials(realm, user, app, owner, "encr_password", retry)
    ...
    def get_all_user_credentials(self, app, timeout=30):
        ...
        search_str = (
            """| rest /servicesNS/nobody/{}/configs/conf-app """  # pylint: disable=consider-using-f-string, duplicate-string-formatting-argument
            """count=0 splunk_server=local """
            """| rename eai:acl.app as app """
            """| where app="{}" """
            """| search password=* """
            '''| rex field=title "credential:(?<realm>.*?):(?<username>[^:]+):$"'''
            r'''| rex field=realm mode=sed "s/\\\:/:/g"'''
        ...
        response, content = sr.simpleRequest(
            path, method="POST", postargs=search, sessionKey=self._session_key
        )
        if response.status == 201:
            ...
            _LOGGER.error(
                "Failed to get user credentials. %d : %s",
                response.status,
                response.reason,
            )
        return None
    ...
    def _wait_search_done(self, sid, timeout):
        ...
        job_id = "/services/search/jobs/{}/".format(  # pylint: disable=consider-using-f-string
            sid
        )
        path = "".join((self._host_path, job_id))
        status_rex = re.compile('isDone">(0|1)')
        for _ in range(timeout):
            response, content = sr.simpleRequest(path, sessionKey=self._session_key)
            if response.status == 200:
    ...
    def _get_search_results(self, sid):
        ...
        """
        results_endpoint = (
            "/services/search/v2/jobs/{}/results?"  # pylint: disable=consider-using-f-string
            "output_format=xml&count=0"
        ).format(sid)
        path = "".join((self._host_path, results_endpoint))
        for _ in range(3):
            response, content = sr.simpleRequest(path, sessionKey=self._session_key)
            if response.status == 200:  # pylint: disable=no-else-break
                break
    ...
    def _get_credentials(
        self, realm, user, app, owner, prop, retry
    ):  # pylint: disable=too-many-arguments
        """
        ...
        for _ in range(retry):
            ...
            try:
                ...
                _LOGGER.error(
                    "Failed to get encrypted password for " "(%s %s %s %s)",
                    realm,
                    user,
                    app,
                    owner,
    ...
    def _escape_string(string_to_escape):
        ...
        escaped string where all the :'s are escaped by a single backslash
