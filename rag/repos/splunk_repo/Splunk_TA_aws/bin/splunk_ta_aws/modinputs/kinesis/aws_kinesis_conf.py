...
#
"""
File for handling AWS kinesis conf inputs.
"""
...
class AWSKinesisConf:
    """Class for AWS kinesis conf."""
    ...
    def _expand_tasks(
        ...
        with logging.LogContext(datainput=stanza[tac.stanza_name]):
            try:
                ...
                akcommon.set_endpoint_urls(stanza)
                try:
                    ...
                    if "ResourceNotFoundException" in str(exc):
                        logger.info(
                            "stream=%s in region=%s has been deleted, please delete this datainput",
                            stream_names,
                            stanza[tac.region],
                        )
                        return []
                        ...
                        logger.error(
                            "Failed to describe stream=%s in region=%s, ignore this datainput. error=%s",
                            stream_names,
                            stanza[tac.region],
                ...
                logger.error(
                    "Failure occurred while expanding tasks for kinesis input, error=%s",
                    traceback.format_exc(),
                )
                return []


class ShardChangesChecker:
    ...
    def check_changes(self):  # pylint: disable=inconsistent-return-statements
        """Returns if changes are present in the stream name."""
        ...
        for stream_name, shards in six.iteritems(stream_shards):
            if len(self._stream_shards[stream_name]) != len(shards):
                logger.info("Detect shard changes for stream_name=%s", stream_name)
                if self._callback is not None:
                    return self._callback()
    ...
    def _get_stream_shards(self):
        ...
        try:
            ...
            if "ResourceNotFoundException" in str(exc):
                ...
                logger.error(
                    "Failed to describe streams, error=%s", traceback.format_exc()
                )
                raise

        stream_shards = {}
        for stream_name, stream in six.iteritems(streams):
