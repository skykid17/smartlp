...
class AWSS3Conf:
    ...
    def _get_tasks(self):
        ...
        for stanza in self.stanza_configs:
            ...
            task["classic_event_writer"] = classic_event_writer
            tasks.append(task)
    ...
    def _get_last_modified_time(self, scan_datetime):
        if not scan_datetime or scan_datetime.strip() == "default":
            ...
            stime = tacommon.parse_datetime(
                self.metas[tac.server_uri], self.metas[tac.session_key], scan_datetime
            )
