...
import six  # fmt: skip
import solnlib.modular_input.event_writer as cew  # fmt: skip
import splunk_ta_aws.common.ta_aws_consts as taconsts  # fmt: skip
...
modular_name = "splunk_ta_aws_cloudtrail"  # pylint: disable=invalid-name
...
modular_args = {
    ...
    "aws_region": {
        ...
        "description": "Name of the AWS region in which the notification queue is "
    ...
    "sqs_queue": {
        ...
        "description": "Name of queue to which notifications of new CloudTrail logs are "
    ...
    "remove_files_when_done": {
        ...
        "description": "Delete log files from the S3 bucket once they have been read "
    ...
    "blacklist": {
        ...
        "description": "PCRE regex for specifying event names to be excluded. "
        "Leave blank to use the default set of read-only event names",
        "required_on_create": False,
    },
    "excluded_events_index": {
        "title": "Excluded events index",
        "description": "Optional index in which to write the excluded events. "
...
class CloudTrailInput(smi.Script):
    """Class for cloudtrail input."""

    def __init__(self):
        super(CloudTrailInput, self).__init__()  # pylint: disable=super-with-arguments
    ...
    def get_scheme(self):
        ...
        # Add arguments
        for name, arg in six.iteritems(modular_args):
            scheme.add_argument(
                smi.Argument(
                    name,
                    title=arg.get("title"),
    ...
    def stream_events(self, inputs, ew):  # pylint: disable=invalid-name
        ...
        logging.setup_root_logger(
            app_name=splunk_ta_aws, modular_name="cloudtrail", stanza_name=stanza_name
        )
        Input(
...
def delete_ckpt(input_name):
    ...
    if op.isfile(ckpt_file):
        os.remove(ckpt_file)
        logger.info(
            "Checkpoint file is deleted: %s"  # pylint: disable=consider-using-f-string
            % ckpt_file
        )


def main():
    """Main method for init module of cloudtrail input."""
