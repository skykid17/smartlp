...
class AccountRestHandler(admin.MConfigHandler):
    ...
    def _getAccountConfig(self, key, exts):
        ...
        if temp:
            if "sts_region" in temp.keys():
                result["skip_flag"] = 1  # Skip Private Account
            for key in temp:
    ...
    def handleCreate(
        ...
        try:
            ...
            cm.create_stanza(
                CONF_FOR_ACCOUNT_EXT_FIELDS, self.callerArgs.id, {"category": cate}
            )
    ...
    def handleEdit(
        ...
        try:
            ...
            new_account_id = account_util.get_account_id(
                {"key_id": key_id, "secret_key": secret_key, "category": category},
                session_key,
            )
            ...
            if cm.stanza_exist(CONF_FOR_ACCOUNT_EXT_FIELDS, account_name):
                cm.update_stanza(
                    CONF_FOR_ACCOUNT_EXT_FIELDS, account_name, {"category": category}
                )
            else:
                cm.create_stanza(
                    CONF_FOR_ACCOUNT_EXT_FIELDS, account_name, {"category": category}
