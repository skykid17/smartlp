...
class BaseModInput(smi.Script):
    ...
    def __init__(self, app_namespace, input_name):
        ...
        try:
            self.setup_util_module = importlib.import_module(
                self.namespace + "_setup_util"
            )
        except ImportError as ie:  # noqa: F841
            self.logger.debug(
                "Can not import package:" + self.namespace + "_setup_util"
            )
    ...
    def parse_input_args(self, inputs):
        raise NotImplemented()  # noqa: F901 # pylint: disable=W0611, E0711, E1102
