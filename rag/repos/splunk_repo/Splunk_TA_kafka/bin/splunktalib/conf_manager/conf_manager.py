...
import splunktalib.conf_manager.request as screq
...
class ConfManager(object):
    ...
    def data_input_exists(self, input_type, name):
        try:
            result = self.get_data_input(input_type, name)
            return result is not None
        except screq.ConfNotExistsException:
            return False
    ...
    def delete_data_input_stanzas(self, input_type, names):
        ...
        for name in names:
            try:
                ...
                logger.error("Unable to delete input_type={} name={}",
                             input_type, name, exc_info=1)
