...
class XMLEventWriter(EventWriter):
    ...
    def write_event(self, data, timestamp=None, **kwargs):
        volume = 0
        ...
        time = "" if not timestamp else self._render_timestamp("time", timestamp)
        data = self._EVENT_TEMPLATE.format(data=data, time=time, **metadata)
        self._write(data)
        logger.debug("Wrote event to STDOUT success.", size=volume)
        return volume
