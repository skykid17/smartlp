...
class ProxyValidator(Validator):
    ...
    def validate(self, value, data):
        """
        Custom proxy validator which checks only for required fields. Proxy information
        will be validated when the API call for fetching the data is performed.
        """
        ...
        try:
            if not data.get("proxy_url"):
                ...
                raise Exception(msg)
            elif (data.get("proxy_username") and not data.get("proxy_password")) or (
                not data.get("proxy_username") and data.get("proxy_password")
            ):
