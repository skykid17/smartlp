...
"""Modular Input Script for Account Admin audit logs"""
import sys

# isort: off
import import_declare_test  # noqa: F401

from cyberark_epm_connect import AccountAdminLogs
...
class ACCOUNT_ADMIN_AUDIT_LOGS(smi.Script):
    def __init__(self):
        self.session_key = None
        super(ACCOUNT_ADMIN_AUDIT_LOGS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme("account_admin_audit_logs")
        scheme.description = "Account Admin Audit Logs"
    ...
    def stream_events(self, inputs, ew):
        ...
        _logger = set_logger(
            self.session_key,
            "splunk_ta_cyberark_epm_account_admin_audit_logs_"
        ...
        try:
            ...
            input_items["sourcetype"] = ACCOUNT_ADMIN_SOURCETYPE
            input_items[
                "collection_name"
            ...
            obj = AccountAdminLogs(config)
            obj.collect_account_admin_audit_logs()
...
if __name__ == "__main__":
    exit_code = ACCOUNT_ADMIN_AUDIT_LOGS().run(sys.argv)
    sys.exit(exit_code)
