...
class GmailLogsMigratedScript(smi.Script):
    def __init__(self):
        super().__init__()

    def get_scheme(self):
        scheme = smi.Scheme("gmail_logs_migrated")
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            try:
                ...
                checkpoint_collection = (
                    f"splunk_ta_google_workspace_gmail_migrated_{normalized_input_name}"
                )
                ...
                table_name = (
                    "activity"
                    if input_item.get("table_name") == None
                    else input_item.get("table_name")
                )
                ...
                query = (
                    f"SELECT gmail.* FROM `{gcp_project_id}.{dataset_name}.{table_name}` "
                    f"WHERE gmail.event_info.timestamp_usec > {start_time_usec} "
                    f"AND gmail.event_info.timestamp_usec < {end_time_usec} "
                    f"AND FORMAT_DATE('%E4Y%m%d', _PARTITIONTIME) >= FORMAT_DATE('%E4Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL 3 DAY)) "
                    f"ORDER BY gmail.event_info.timestamp_usec ASC"
                )
...
if __name__ == "__main__":
    exit_code = GmailLogsMigratedScript().run(sys.argv)
    sys.exit(exit_code)
