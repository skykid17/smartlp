...
def splunk_rest_request(path, sessionKey=None, getargs=None, postargs=None, method='GET', raiseAllErrors=False, proxyMode=False, rawResult=False, timeout=30, jsonargs=None):
	...
	ssl_cert = rest.getWebCertFile()
	ssl_key = rest.getWebKeyFile()
	if cert_validation:
		logger.info(
...
class splunk_for_vmware_setup(controllers.BaseController):
	...
	def validate_collection_node(self, app, action, **kwargs):
		...
		try:
			...
			if remote_session_key is None:
				...
				required_apps = ["SA-Hydra", "Splunk_TA_vmware"]
				installed_count = 0
				installed_apps = []
				for app in apps:
	...
	def validate_unmanaged_host(self, app, action, **kwargs):
		...
		if host:
			validated_hostincludelist = re.search("[A-Za-z0-9\.\-_]", host)
			validated_hostexcludelist = re.search("[\r\n\t]", host)
			if validated_hostincludelist is None or validated_hostexcludelist is not None:
		...
		if name:
			validated_nameallowlist = re.search("[A-Za-z0-9\.\-_]", name)
			validated_namedenylist = re.search("[\r\n\t]", name)
			if validated_nameallowlist is None or validated_namedenylist is not None:
	...
	def _get_filtered_host_info(self, connection, vc_stanza):
		...
		into included and excluded lists as per the allow/denylists in the ta_vmware_collection.conf stanza
		Args:
		...
		if (vc_stanza.managed_host_excludelist is not None) and (vc_stanza.managed_host_excludelist != "None"):
			exclude_re_search = re.compile(vc_stanza.managed_host_excludelist, flags=re.S).search
		else:
			#fake re search method, always doesn't match
			exclude_re_search = lambda s: None
		if (vc_stanza.managed_host_includelist is not None) and (vc_stanza.managed_host_includelist != "None"):
			include_re_search = re.compile(vc_stanza.managed_host_includelist, flags=re.S).search
		else:
		...
		for host in host_list:
			...
			if exclude_re_search(host['name']) or (include_re_search(host['name']) is None):
		...
		if remote_session_key is None:
			...
			for counter in range(len(input_names)):
				...
				try:
					...
					message = "Problem editing the number of worker inputs on the remote node={0}: ".format(node_path) + str(e)
	...
	def delete_collection_node(self, app, action, node_path, **kwargs):
		...
		if remote_session_key is None:
			...
			try:
				...
				message = "Problem deleting the stored credentials on the remote node={0}: ".format(node_path) + str(e)
				logger.exception(message)
	...
	def save_vcenter(self, app, action, **kwargs):
		...
		REQUEST PARAMS:
			...
			OPTIONAL:
				...
				host_includelist - the host includelist to use, if null no allowlist will be used
				host_excludelist - the host excludelist to use, if null no denylist will be used
		...
		host_includelist = kwargs.get("host_includelist", "")
		host_excludelist = kwargs.get("host_excludelist", "")
		...
		vc_stanza.managed_host_excludelist = host_excludelist
		vc_stanza.managed_host_includelist = host_includelist
	...
	def save_collection(self, *args, **kwargs):
		...
		collection_metric_list = json.loads(kwargs.get("collection_metric_list", ""))

		# Validation begins from here.
		for task_, interval in interval_dict.items():
			try:
				if int(interval) < 0:
					logger.error("Could not save configuration, interval value passed for {0} task is not valid.".format(task_.split("_interval")[0]))
					raise VMwareSetupError(status=500, message="Interval value passed to task interval is not valid.")
			except ValueError:
				# This is to raise error for non-integer inputs
				logger.error("Could not save configuration, interval value passed for {0} task is not valid.".format(task_.split("_interval")[0]))
				raise VMwareSetupError(status=500, message="Interval value passed to task interval is not valid.")
			
		for task_, expiration in expiration_dict.items():
			try:
				if int(expiration) < 0:
					logger.error("Could not save configuration, expiration value passed for {0} task is not valid.".format(task_.split("_expiration")[0]))
					raise VMwareSetupError(status=500, message="Expiration value passed to expiration value is not valid.")
			except ValueError:
				# This is to raise error for non-integer inputs
				logger.error("Could not save configuration, expiration value passed for {0} task is not valid.".format(task_.split("_expiration")[0]))
				raise VMwareSetupError(status=500, message="Expiration value passed to task expiration is not valid.")
		...
		for key in list(expiration_dict.keys()):
			stanza_dict[key] = expiration_dict[key]

		collection_instance_value = {0: "", 1: ".*"}

		for key in list(collection_instance_dict.keys()):
			stanza_dict[key] = collection_instance_value[collection_instance_dict[key]]

		for key in list(collection_metric_list.keys()):
			stanza_dict[key] = collection_metric_list[key]
	...
	def get_collection(self, *args, **kwargs):
		...
		interval_list = []
		expiration_list = []
		collection_metric_dict = {}
		entity_madl_list = ["vm_metric_allowlist", "vm_metric_denylist", "host_metric_allowlist", "host_metric_denylist", "cluster_metric_allowlist", "cluster_metric_denylist", "rp_metric_allowlist", "rp_metric_denylist"]
		...
		try:
			error_check = default_stanza.host_instance_allowlist[0]
		except:
			default_stanza.host_instance_allowlist = [""]

		try:
			error_check = default_stanza.vm_instance_allowlist[0]
		except:
			default_stanza.vm_instance_allowlist = [""]
		...
		try:
			...
			default_dict["collection_instance_list"] = [
				collection_instance_dict.get(default_stanza.host_instance_allowlist[0], "true"),
				collection_instance_dict.get(default_stanza.vm_instance_allowlist[0], "true")]
		except Exception as e:
			logger.error(str(e))
		...
		try:
			for entity_madl in entity_madl_list:
				madl_list = getattr(default_stanza, entity_madl, [""])
				madl = ', '.join(madl_list)
				collection_metric_dict[entity_madl] = madl
			default_dict["collection_metric_dict"] = collection_metric_dict
		except Exception as e:
			logger.error(str(e))
	...
	def _get_full_conf_data(self):
		...
		for stanza in stanzas:
			if stanza.target_type == "vc":
				if len(stanza.target) != 1:
					...
					tmp["managed_host_excludelist"] = stanza.managed_host_excludelist
					tmp["managed_host_includelist"] = stanza.managed_host_includelist
		...
		for node in conf_data["nodes"]:
			node["username"] = html.escape(node["username"])
		for vcenter in conf_data["vcenters"]:
			vcenter["username"] = html.escape(vcenter["username"])
		...
		if remote_session_key is None:
			...
			for uri in input_uris:
				...
				try:
					...
					message = "Problem editing inputs on the remote vc_splunk_forwarder={0}: ".format(host_path) + str(e)
					logger.exception(message)
					status = False
		return status
