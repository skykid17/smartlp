...
class MetricsListMatcher(object):
    """Class to perform allowlist/denylist selections.
    
    Instantiated with kwargs:
      allowlist - list of regex or verbatim strings
      denylist - list of regex or verbatim strings
      mode ["verbatim" | "regex"] - matching mode
    
    Lists are filtered using the prune method."""
    def __init__(self, allowlist=None, denylist=None, mode="verbatim"):
        self.w = set(allowlist) if bool(allowlist) else None
        self.b = set(denylist) if bool(denylist) else None
        self.mode = mode
    ...
    def prune(self, l, name_extractor=lambda x: x, return_excluded=False):
        """Filter a list based on allowlists/denylists.
        ...
        Optional arguments: 
          ...
          return_excluded (boolean) - if True, separate the original list into 
              two, one with items that conform to the allow/denylists, and the other 
