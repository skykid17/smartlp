   ...
   <xsl:apply-imports />
        <xsl:for-each select="syslog/audit_record">CEF:0|<xsl:value-of select="Vendor"/>|<xsl:value-of select="Product"/>|<xsl:value-of select="Version"/>|<xsl:value-of select="MessageID"/>|<xsl:choose><xsl:when test="Severity='Critical' or Severity='Error'">Failure: </xsl:when></xsl:choose><xsl:call-template name="string-replace">
            ...
            <xsl:otherwise><xsl:for-each select="CAProperties/CAProperty"><xsl:if test="@Name='UserName'"><xsl:call-template name="string-replace">
                ...
                <xsl:with-param name="to" select="';'"/> 
                <xsl:with-param name="string" select="ExtraDetails"/>
            </xsl:call-template> cs1Label="Affected User Name" cs1=<xsl:if test="/syslog/audit_record/sourceUser !=''">"<xsl:call-template name="string-replace">
            <xsl:with-param name="from" select="'='"/>
            <xsl:with-param name="to" select="'\='"/> 
            <xsl:with-param name="string" select="SourceUser"/>
        </xsl:call-template>"</xsl:if> cs2Label="Safe Name" cs2=<xsl:if test="/syslog/audit_record/Safe != ''">"<xsl:call-template name="string-replace">
            <xsl:with-param name="from" select="'='"/>
            <xsl:with-param name="to" select="'\='"/> 
            <xsl:with-param name="string" select="Safe"/>
        </xsl:call-template>"</xsl:if> cs3Label="Device Type" cs3=<xsl:for-each select="CAProperties/CAProperty"><xsl:if test="@Name='DeviceType'">"<xsl:call-template name="string-replace">
            <xsl:with-param name="from" select="'='"/>
            <xsl:with-param name="to" select="'/='"/> 
            <xsl:with-param name="string" select="@Value"/>
        </xsl:call-template>"</xsl:if></xsl:for-each> cs4Label="Database" cs4=<xsl:if test="contains(/syslog/audit_record/ExtraDetails,'Database=')">"<xsl:call-template name="string-GetValue">
            <xsl:with-param name="from" select="'DataBase='"/>
            <xsl:with-param name="to" select="';'"/> 
            <xsl:with-param name="string" select="ExtraDetails"/>
        </xsl:call-template>"</xsl:if> cs5Label="Other info" cs5=<xsl:if test="/syslog/audit_record/Location != '' or /syslog/audit_record/Category != '' or /syslog/audit_record/GatewayStation != '' ">"<xsl:call-template name="string-replace">
            <xsl:with-param name="from" select="'='"/>
            <xsl:with-param name="to" select="'\='"/> 
            ...
            <xsl:with-param name="to" select="'\='"/> 
            <xsl:with-param name="string" select="GatewayStation"/>   
        </xsl:call-template>"</xsl:if> cn1Label="Request Id" cn1=<xsl:if test="/syslog/audit_record/RequestId != ''">"<xsl:value-of select="RequestId"/>"</xsl:if> cn2Label="Ticket Id" cn2=<xsl:if test="/syslog/audit_record/Reason != ''">"<xsl:value-of select="Reason"/>"</xsl:if>  msg=<xsl:if test="/syslog/audit_record/Reason != ''">"<xsl:call-template name="string-replace">
            <xsl:with-param name="from" select="'='"/>
            <xsl:with-param name="to" select="'\='"/> 
