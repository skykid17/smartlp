...
import subprocess  # nosemgrep: gitlab.bandit.B404 - module usage is not controllable by an external resource
import sys
import traceback
...
def _parse_modinput_configs(root, outer_block, inner_block):
    ...
    if not confs:
        _LOGGER.error("Invalid config, missing {} section".format(outer_block))
...
def parse_modinput_configs(config_str):
    ...
    for tag in meta_configs.keys():
        ...
        if not nodes:
            _LOGGER.error("Invalid config, missing {} section".format(tag))
            raise Exception("Invalid config, missing {} section".format(tag))

        if nodes[0].firstChild and nodes[0].firstChild.nodeType == nodes[0].TEXT_NODE:
            meta_configs[tag] = nodes[0].firstChild.data
        else:
            _LOGGER.error("Invalid config, expect text node")
            raise Exception("Invalid config, expect text node")
...
def get_modinput_configs(modinput, modinput_stanza=None):
    ...
    (
        out,
        err,
    ) = subprocess.Popen(  # nosemgrep: python.lang.security.audit.dangerous-subprocess-use.dangerous-subprocess-use
        cli,
    ...
    def encrypt_new_credentials(self, account, conf_file, host_user_passwds):
        ...
        for url_k, user_k, passwd_k in host_user_passwds:
            ...
            if (
                ...
                _LOGGER.info("Encrypt credential for {}".format(url))
