...
class KVClient:
    ...
    def _do_request(
        ...
        response = rest.splunkd_request(uri, self._session_key, method, headers, data)
        if response is None and not response.content:
        ...
        elif response and response.status_code in (500, 404):
            raise KVNotExists("{0}-{1} not exists".format(uri, data))
        else:
            raise KVException(
                "Failed to {0} {1}, reason={2}".format(method, uri, response.reason)
            )
