...
import ta_util2.credentials as cred
...
def _parse_modinput_configs(root, outer_block, inner_block):
    ...
    if not confs:
        ...
        raise Exception("Invalid config, missing %s section", outer_block)
...
def get_modinput_configs(modinput, modinput_stanza=None):
    ...
    ).communicate()  # nosemgrep false-positive: The vulnerable method get_modinput_configs in the framework file is not getting used anywhere in the add-on v8.1.0 # noqa: E501
    if err:
...
class TAConfig:
    ...
    def get_configs(self):
        ...
        self.cred_manager = cred.CredentialManager(
            meta_configs["session_key"], meta_configs["server_uri"]
        )
    ...
    def encrypt_new_credentials(self, account, conf_file, host_user_passwds):
        ...
        for (url_k, user_k, passwd_k) in host_user_passwds:
            ...
            if (
                ...
                and username
                and password
                and username != encrypted
                and password != encrypted
            ):
                # Create new one
                user_password = sep.join((username, password))
                res = self.cred_manager.update(url, self.dummy, user_password, self.app)
                if not res:
    ...
    def encrypt_conf_file(self, conf_file):
        ...
        for d in ("local", "default"):
            ...
            encrypted_file = op.join(self.app_dir, d, ".{0}.new".format(conf_file))
    ...
    def decrypt_existing_credentials(self, defaults, host_user_passwds):
        ...
        for (url_k, user_k, passwd_k) in host_user_passwds:
            ...
            if (
                ...
                and username
                and password
                and username == self.encrypted
                and password == self.encrypted
            ):
                password = self.cred_manager.get_clear_password(
                    url, self.dummy, self.app
                )
                if not password:
...
def reload_confs(confs, session_key, splunkd_uri="https://localhost:8089", appname="-"):
    ...
    for conf in new_confs:
        ...
        response = rest.splunkd_request(endpoint, session_key)
        if response and response.status_code not in (200, 201):
            _LOGGER.error("Failed to refresh %s, reason=%s", endpoint, response.reason)
...
class ConfManager:
    ...
    def create_data_input(self, user, appname, input_type, stanza):
        ...
        msg = "Failed to create data input for {}, {}".format(appname, input_type)
    ...
    def _disable_enable_data_input(
        self, user, appname, input_type, stanza_name, disable_enable
    ):
        uri = "{}/servicesNS/{}/{}/data/inputs/{}/{}/{}".format(
            self.splunkd_uri, user, appname, input_type, stanza_name, disable_enable
        )
    ...
    def _do_request(self, uri, method, payload, err_msg):
        ...
        if response and response.status_code in (200, 201):
            ...
            _LOGGER.debug("%s, reason=%s", err_msg, response.reason)
        return None
