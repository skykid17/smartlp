...
class StateStore:
    def __init__(self, meta_configs, appname, use_kv_store=False):
        ...
        if utils.is_true(use_kv_store):
            ...
            kvc.create_collection(
                self._kv_client, self._state_collection, self._appname
            )
    ...
    def delete_state(self, key):
        if self._kv_client:
            ...
            self._kv_client.delete_collection_data(
                self._state_collection, self._states_cache[key]["_key"], self._appname
            )
    ...
    def _pop_states_cache(self):
        states = self._kv_client.get_collection_data(
            self._state_collection, None, self._appname
        )
        if not states:
            return
