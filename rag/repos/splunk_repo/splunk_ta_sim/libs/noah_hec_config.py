...
class NoahHECConfig:
    ...
    @retry(exceptions=[binding.HTTPError])
    def create_input(self, name: str, stanza: dict, headers=None) -> dict:
        ...
        Examples:
           >>> from solnlib.hec_config import NoahHECConfig
           >>> hec = NoahHECConfig(session_key)
           >>> hec.create_input('my_hec_data_input',
                                {'index': 'main', 'sourcetype': 'hec'})
        """

        res = self._rest_client.inputs.create(name, self.input_type, headers=headers, **stanza)
        return res.content
