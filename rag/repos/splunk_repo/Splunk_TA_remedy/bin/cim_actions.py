...
"""
* flake8 ignores:
- noqa: E203 -> Def = Whitespace before ':'
    Reason for ignoring = False positive
- noqa: E731 -> do not assign a lambda expression, use a def
    Reason = Here, we'll have to use lambda func only.
- noqa: W605 -> Invalid escape sequence
    Reason = Ignore for now. Will create a backlog ticket.
"""
...
class ModularAction:
    ...
    def __init__(self, settings, logger, action_name="unknown"):
        ...
        try:
            rtsid = re.match("^(rt_scheduler.*)\.(\d+)$", self.sid)  # noqa: W605
            if rtsid:
    ...
    def message(self, signature, status=None, rids=None, level=logging.INFO, **kwargs):
        ...
        for rid_ntuple in rids:
            if len(rid_ntuple) == 3:
                ...
                for match in re.finditer('[A-Za-z_]+=""(\s|$)', message):  # noqa: W605
    ...
    def writeevents(
        ...
        if self.events:
            ...
            if not fext or not re.match("^[\w-]+$", fext):  # noqa: W605
                self.logger.warn(
            ...
            for chunk in (
                self.events[x : x + ModularAction.DEFAULT_CHUNK]  # noqa: E203
